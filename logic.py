...
from flask import Flask, jsonify, request, render_template, url_for, send_from_directory, Markup, Response
import pandas as pd
...
static_dir = os.path.abspath(os.path.dirname(os.path.realpath(__file__))+'/static')
...
DEBUG = True
TEST_IMAGES_DIR = "/mnt/sample_images/"
LABEL_PATH = "/mnt/sample_images/Data_Entry_2017.csv"
#LABEL_PATH = os.path.abspath(os.path.dirname(os.path.realpath(__file__))+'/sample_images/Data_Entry_2017.csv')
MODEL_PATH = "/mnt/model/model.bigdl"
MODEL_WEIGHTS_PATH = "/mnt/model/model.bin"
...
def display_static():
    filename = request.args.get('filename', None)
    return send_from_directory("/mnt/flask_app/", filename)
...
@app.route('/model_summary', methods=['GET', 'POST'])
def model_details():
    if request.method == 'GET' or request.method == 'POST' :
        #summary_file = 'model.png'
        # show the upload form
        return render_template("model_details.html")
        
 
@app.route('/model_accuracy', methods=['GET', 'POST'])
def model_accuracy():
    if request.method == 'GET' or request.method == 'POST' :
        #summary_file = 'accuracy.png'
        # show the upload form
